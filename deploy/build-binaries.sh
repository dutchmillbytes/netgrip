...
VERSION="${VERSION:-dev}"
RELEASE_REPO="${RELEASE_REPO:-gnacho/netgrip}"
...
build() {
  ...
  env \
    ...
    "$GO_BIN" build \
      -trimpath \
      -buildvcs=false \
      -ldflags="-s -w -buildid= -X main.version=${VERSION} -X github.com/gnacho/netgrip/internal/modules.releaseArch=${release_arch} -X github.com/gnacho/netgrip/internal/modules.githubRepo=${RELEASE_REPO}" \
